...
public class Jeu extends MouseAdapter implements ActionListener, WindowListener {
	...
	/**
	 * Taille en hauteur de la fenétre en px.
	 */
	private int fenetreH;
	
	/**
	 * Taille en longueur de la fenétre en px.
	 */
	...
	/**
	 * etiquette est un tableau é 2 dimensions d'etiquettes définissant toutes les cases du plateau du jeu.
	 */
	...
	/**
	 * tabNumeros est un tableau contenant les images des numéros des cases de 0 é 8.
	 */
	...
	/**
	 * Tableau é 2 dimensions des numéros des cases : 
	...
	/**
	 * Tableau é 2 dimensions des cases cliquées : 
	 ...
	 * du clic gauche sur une case pour connaétre l'ordonnée de la case.
	 */
	private int sauv_i;
	
	/**
	 * sauv_j est utilisée pour sauvegarder la valeur de j lors
	 * du clic gauche sur une case pour connaétre l'abscisse de la case.
	 */
	private int sauv_j;
	...
	/**
	 * cases est le panneau de la grille de jeu qui est affiché é l'écran.
	 */
	...
	/**
	 * Méme commentaire que pour menuPartie mais désactive le menu Aide.
	 */
	private JMenu menuAide;
	
	/**
	 * Fenétre principale du jeu.
	 */
	private JFrame fenetre;
	
	/**
	 * Timer permettant de laisser le temps de voir l'explosion de la mine avant d'afficher la boéte de dialogue.
	 */
	...
	/**
	 * SimpleDateFormat décrit le format de l'horloge é afficher en l'occurrence HH:mm:ss ici.
	 */
	...
	/**
	 * horloge est l'étiquette de l'horloge qui sera affichée é l'écran et actualisée toutes les secondes.
	 */
	...
	/**
	 * timerPause permet de savoir quand l'horloge doit se mettre en pause (perte du focus et réduction de la fenétre).
	 */
	 ...
	 * @param fenetreH Taille en hauteur de la fenétre en px.
	 * @param fenetreL Taille en longueur de la fenétre en px.
	 */
	...
	/**
	 * Constructeur Jeu pour reprendre la derniére partie.
	 *
	 * @param tabCases_lecture Meme tableau que tabCases mais contient les valeurs enregistrées du fichier de sauvegarde.
	 * @param tabCasesFlags_lecture Méme tableau que tabCasesFlags mais contient les valeurs enregistrées du fichier de sauvegarde.
	 ...
	 * @param fenetreH_lecture Taille en hauteur de la fenétre en px de la sauvegarde.
	 * @param fenetreL_lecture Taille en longueur de la fenétre en px de la sauvegarde.
	...
	public Jeu(int[][] tabCases_lecture, int[][] tabCasesFlags_lecture, int tailleH_lecture, int tailleL_lecture, int fenetreH_lecture, int fenetreL_lecture, long tempsMs_lecture) {
		...
		charger = true; // Ne pas initialiser les tableaux et découvrir les cases déjé cliquées
		timerPause = false;
	}
	...
	/**
	 * Initialise tout le jeu et l'affiche é l'écran.
	 */
	public void afficher() {
		fenetre = new JFrame("EL Anewar DAMALA Aminata lo Mbaye ");
	...
	private void initPlateauJeu() {
		...
		imageMine = new ImageIcon(assetsPath + "drapeau_petit.png");
		imageDrapeau = new ImageIcon(assetsPath + "mine_petit.png");
		imagePointInterr = new ImageIcon(assetsPath + "point_interrogation_petit.png");
		
		tabNumeros = new ImageIcon[9]; // Création d'un tableau d'icones contenant les icones des chiffres de 0 é 8
		for (int i = 0; i < 9; i++) {
		...
		// Découverte des cases cliquées lors de la restauration de la derniére partie
		if(charger) {
			for(int i = 0; i < tailleH; i++) {
				for(int j = 0; j < tailleL; j++) {
	...
	private void initBarreMenu() {
		...
		ssMenu_reprendre = new JMenuItem("");
		ssMenu_sauverEtQuitter = new JMenuItem("");
		ssMenu_sauverEtQuitter.setEnabled(false);
		JMenuItem ssMenu_quitter = new JMenuItem("");
		JMenuItem ssMenu_aPropos = new JMenuItem("");
		...
		if(fichierSauvegarde_bouton.exists()) { // Activation du bouton "Reprendre la derniére partie sauvegardée" si une sauvegarde existe
	...
	private void initMenuMinesRestantes() {
		...
		if (!charger) {
			tempsMs = -3600000; // 01:00:00 si tempsMs = 0 moins 3600000ms pour commencer é 00:00:00
		}
		else {
	...
	private void initTableaux() {
		...
		for(int i = 0; i < tailleH; i++) { // Initialisation du tableau é 0;
			for(int j = 0; j < tailleL; j++) {
				tabCasesFlags[i][j] = 0;
			}
		}
		...
		for (int i = 0; i < tabMines.length; i++) { 
			// Initialisation de tabMines é -1 pour éviter que les 0 soient comptés comme des mines
			tabMines[i] = -1;
		}
		
		for(int i = 0; i < tabMines.length; i++) { // Remplissage du tableau tabMines 
			// Boucle tant que la valeur tirée existe dans le tableau pour ne pas avoir plusieurs mines dans la méme case
			do {
		...
		for(int i = 0; i < tailleH; i++) {
			for(int j = 0; j < tailleL; j++) {
				if(tabCases[i][j] != 9) {
					...
					contenant maintenant les cases minées notées 9 et non minées notées de 0 é 8 */
	 ...
	 * 
	 * @param tab[] Le tableau é parcourir.
	 * @param valeur La valeur é comparer aux valeurs du tableau.
	...
	/**
	 * La méthode valeurExisteDansTableau2() vérifie si une valeur existe dans un tableau é 2 dimensions.
	 * 
	 * @param tab[][] Le tableau é parcourir.
	 * @param valeur La valeur é comparer aux valeurs du tableau.
	 * @return vrai si la valeur est dans le tableau ou faux si elle n'y est pas.
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getActionCommand() == "Recommencer") {
			...
			dialogueGagnerPerdre("Partie perdue", "Vous avez perdu cette partie.");
			...
			if (!timerPause) {
				if (tempsMs >= 82799000) { // Correspond a 23:59:59
					ActionListener[] timerListener = timerTemps.getActionListeners();
					timerTemps.removeActionListener(timerListener[0]); // Supprime le listener lorsque l'horloge est é 23:59:59
				}
				tempsMs += 200;
	...
	/**
	 * La méthode initNbMinesRestantes() calcule le nombre de mines restantes lors du début ou é la restauration d'une partie.
	 */
	...
	/**
	 * La méthode decouvrirCases() permet de découvrir les 8 cases adjacentes é la case cliquée et aux cases vides.
	 * 
	...
	/**
	 * La méthode perdre() révéle toutes les cases de la grille en affichant en rouge les cases minées, en noir la case minée cliquée,
	 * en bleu péle les cases non découvertes et en affichant une croix rouge sur les cases non minées marquées.
	 * Puis elle affiche une boéte de dialogue permettant de créer une nouvelle partie, recommencer une partie ou quitter le jeu.
	 * 
	...
	private void perdre(int i_mineCliquee, int j_mineCliquee) {
		...
		for(int i = 0; i < tailleH; i++) {
			for(int j = 0; j < tailleL; j++) {		
				if (tabCases[i][j] == 9) {
					...
					etiquette[i][j].setBackground(new Color(120, 120, 255)); // Case non découverte en bleu péle et affichage du numéro
		...
		Image mineExplosion_gif = Toolkit.getDefaultToolkit().createImage(assetsPath + "mine_petit.png");
		...
		timerPerdre = new Timer(1200, this); // Timer d'1,2s pour voir l'explosion de la mine avant d'afficher la boéte de dialogue
	...
	/**
	 * La méthode gagner() révéle toutes les cases de la grille en affichant en vert les cases minées
	 * puis elle affiche une boéte de dialogue permettant de créer une nouvelle partie, recommencer une partie,
	...
	/**
	 * La méthode dialogueGagnerPerdre() affiche la boéte de dialogue lorsqu'on gagne ou on perd.
	 * 
	 * @param titre le titre de la boéte de dialogue
	...
	/**
	 * La méthode windowClosing() est appelée lors de la fermeture de la fenétre
	...
	/**
	 * La méthode windowDeactivated() est appelée lors de la perte du focus de la fenétre et permet de mettre en pause le chronométre.
	 * 
	 * @param e Ce paramétre décrit la source et les circonstances de l'événement.
	 */
	@Override
	...
	/**
	 * La méthode windowIconified() est appelée lorsque la fenétre se minimise et permet de mettre en pause le chronométre.
	 * 
	 * @param e Ce paramétre décrit la source et les circonstances de l'événement.
	 */
	@Override
	...
	/**
	 * La méthode windowActivated() est appelée lorsque la fenétre revient au premier plan et permet de réactiver le chronométre.
	 * 
	 * @param e Ce paramétre décrit la source et les circonstances de l'événement.
	 */
	@Override
	...
	/**
	 * La méthode windowDeiconified() est appelée lorsque la fenétre revient
	 * au premier plan alors qu'elle était minimisée et permet de réactiver le chronométre.
	 * 
	 * @param e Ce paramétre décrit la source et les circonstances de l'événement.
	 */
	@Override
	...
	/**
	 * La méthode windowClosed() est appelée lorsque la fenétre est fermée mais elle n'est pas utilisée ici.
	 * 
	 * @param e Ce paramétre décrit la source et les circonstances de l'événement.
	 */
	@Override
	...
	/**
	 * La méthode windowOpened() est appelée lorsque la fenétre est ouverte mais elle n'est pas utilisée ici.
	 * 
	 * @param e Ce paramétre décrit la source et les circonstances de l'événement.
	 */
	@Override
